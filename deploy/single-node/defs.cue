...
k8s_list: [
	...
	(clickhouse & {
		#Config: {
			clickhouseOperatorPassword: netmeta.config.clickhouseOperatorPassword
			enableClickhouseIngress:    netmeta.config.enableClickhouseIngress
...
k8s: close({
	...
	PersistentVolumeClaim: [NANE=string]: core_v1.#PersistentVolumeClaim & {apiVersion: "v1"}
...
objects: [ for _, v in k8s for _, x in v {x}]

// Prerequisite objects to apply first, in a separate kubectl call.
preObjects: [ for v in objects if v.kind != "CustomResourceDefintion" {v}]
