...
AS
SELECT * REPLACE (
    if(dictGet('SamplerConfig', 'CensorAddresses', IPv6NumToString(SamplerAddress)), cutIPv6(SrcAddr, 8, 1), SrcAddr) AS SrcAddr,
    if(dictGet('SamplerConfig', 'CensorAddresses', IPv6NumToString(SamplerAddress)), cutIPv6(DstAddr, 8, 1), DstAddr) AS DstAddr
    )
FROM (
