...
func (ti *tapInterface) Run(ctx context.Context, wg *sync.WaitGroup, kafkaState *transport.KafkaState) {
	...
	for seqNum := 0; true; seqNum++ {
		...
		if ti == ti.pair.RX {
			inIf = uint32(*ti.pair.RX.sysIface.IfIndex)
			outIf = uint32(*ti.pair.TX.sysIface.IfIndex)
		} else {
			inIf = uint32(*ti.pair.TX.sysIface.IfIndex)
			outIf = uint32(*ti.pair.RX.sysIface.IfIndex)
		}
