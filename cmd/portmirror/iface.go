...
func loadConfig() ([]*tapPair, error) {
	if len(interfaces) == 0 {
		return nil, fmt.Errorf("please provide interface pairs")
	}

	var tapPairs []*tapPair
	for _, v := range strings.Split(interfaces, ",") {
		if !strings.Contains(v, ":") {
			return nil, fmt.Errorf("invalid interface pair given: %q", v)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("cant find interface: %q", rxName)
		}

		if err := netlink.SetPromiscOn(rxLink); err != nil {
			return nil, fmt.Errorf("cant set promisc mode on interface: %q", rxName)
		}

		txLink, err := netlink.LinkByName(txName)
		if err != nil {
			return nil, fmt.Errorf("cant find interface: %q", txName)
		}

		if err := netlink.SetPromiscOn(txLink); err != nil {
			return nil, fmt.Errorf("cant set promisc mode on interface: %q", txName)
		}
